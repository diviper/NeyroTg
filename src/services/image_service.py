...
class ImageService:
    ...
    def generate_description(self, prompt, style="default"):
        """Генерация улучшенного описания через OpenAI API"""
        style_hint = ""
        if style != "default":
            style_prompt = self.style_prompts.get(style)
            if style_prompt:
                style_hint = f"\nСтиль изображения: {style_prompt}"

        if not prompt or prompt.isspace():
            # Если текст пустой, генерируем случайное описание
            base_prompt = f"""Создай интересное описание для случайной сцены, которое подходит для генерации изображения.
            Описание должно быть детальным, но не длиннее 2-3 предложений.{style_hint}"""
        else:
            ...
            5. Оставь длину в пределах 2-3 предложений{style_hint}
